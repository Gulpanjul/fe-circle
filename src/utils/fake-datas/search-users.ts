import { type SearchUser } from '@/features/search-users/types/search-user';

export const searchUserDatas: SearchUser[] = [
    {
        id: '8d5e7a20-1b63-11ee-be56-0242ac120002',
        fullName: '08 Endranio Palupi',
        username: 'endranio_palupi',
        avatarUrl:
            'https://api.dicebear.com/9.x/notionists/svg?seed=08%20Endranio%20Palupi',
        bio: 'Passionate about technology and innovation.',
        isFollowed: true,
    },
    {
        id: '8d5e7b90-1b63-11ee-be56-0242ac120003',
        fullName: 'Iqbal M Hasbi',
        username: 'iqbal_hasbi',
        avatarUrl:
            'https://api.dicebear.com/9.x/notionists/svg?seed=Iqbal%20M%20Hasbi',
        bio: 'Lover of books and deep conversations.',
        isFollowed: false,
    },
    {
        id: '8d5e7cf0-1b63-11ee-be56-0242ac120004',
        fullName: 'M Rizal',
        username: 'm_rizal',
        avatarUrl: 'https://api.dicebear.com/9.x/notionists/svg?seed=M%20Rizal',
        bio: 'Traveler and photographer.',
        isFollowed: true,
    },
    {
        id: '8d5e7e50-1b63-11ee-be56-0242ac120005',
        fullName: 'Muh. Yaumil Aksah Hamid',
        username: 'yaumil_aksah',
        avatarUrl:
            'https://api.dicebear.com/9.x/notionists/svg?seed=Muh.%20Yaumil%20Aksah%20Hamid',
        bio: 'Aspiring writer and storyteller.',
        isFollowed: false,
    },
    {
        id: '8d5e7fba-1b63-11ee-be56-0242ac120006',
        fullName: 'Muhammad Alfiandi Rizki',
        username: 'alfiandi_rizki',
        avatarUrl:
            'https://api.dicebear.com/9.x/notionists/svg?seed=Muhammad%20Alfiandi%20Rizki',
        bio: 'Gamer and tech enthusiast.',
        isFollowed: true,
    },
    {
        id: '8d5e8126-1b63-11ee-be56-0242ac120007',
        fullName: 'Paste Prosman',
        username: 'paste_prosman',
        avatarUrl:
            'https://api.dicebear.com/9.x/notionists/svg?seed=Paste%20Prosman',
        bio: 'Coding is my superpower.',
        isFollowed: false,
    },
    {
        id: '8d5e828c-1b63-11ee-be56-0242ac120008',
        fullName: 'Rahim Haq',
        username: 'rahim_haq',
        avatarUrl:
            'https://api.dicebear.com/9.x/notionists/svg?seed=Rahim%20Haq',
        bio: 'Music and art lover.',
        isFollowed: true,
    },
    {
        id: '8d5e83fc-1b63-11ee-be56-0242ac120009',
        fullName: 'Rendy Zulfan',
        username: 'rendy_zulfan',
        avatarUrl:
            'https://api.dicebear.com/9.x/notionists/svg?seed=Rendy%20Zulfan',
        bio: 'Entrepreneur and digital marketer.',
        isFollowed: false,
    },
    {
        id: '8d5e8574-1b63-11ee-be56-0242ac120010',
        fullName: 'Syifa Maulaya',
        username: 'syifa_maulaya',
        avatarUrl:
            'https://api.dicebear.com/9.x/notionists/svg?seed=Syifa%20Maulaya',
        bio: 'Lover of coffee and cozy places.',
        isFollowed: true,
    },
    {
        id: '8d5e86e2-1b63-11ee-be56-0242ac120011',
        fullName: 'Tajjuddin Auliya Ahaadiin',
        username: 'tajjuddin_auliya',
        avatarUrl:
            'https://api.dicebear.com/9.x/notionists/svg?seed=Tajjuddin%20Auliya%20Ahaadiin',
        bio: 'Always learning, always growing.',
        isFollowed: false,
    },
];
